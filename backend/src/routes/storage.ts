...
// Upload bus image
// @ts-ignore
router.post('/upload/bus-image', authenticateUser, requireAdmin, upload.single('image'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    const uploadResult = await StorageService.uploadImage(req.file, 'buses', `bus_${busId}`);
...
// @ts-ignore
router.post('/upload/driver-photo', authenticateUser, requireAdmin, upload.single('photo'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    const uploadResult = await StorageService.uploadImage(req.file, 'drivers', `driver_${driverId}`);
...
// Upload route map
// @ts-ignore
router.post('/upload/route-map', authenticateUser, requireAdmin, upload.single('map'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    const uploadResult = await StorageService.uploadDocument(req.file, 'routes', `route_${routeId}`);
...
// Delete file
router.delete('/delete/:type/:id', authenticateUser, requireAdmin, async (req, res) => {
  try {
    ...
    switch (type) {
      case 'bus':
        updateQuery = 'UPDATE buses SET bus_image_url = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = $1';
        updateParams = [id];
        break;
      case 'driver':
        updateQuery = 'UPDATE users SET profile_photo_url = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND role = $2';
        updateParams = [id, 'driver'];
        break;
      case 'route':
        updateQuery = 'UPDATE routes SET route_map_url = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = $1';
        updateParams = [id];
        break;
      default:
...
// Get file info
router.get('/info/:type/:id', authenticateUser, requireAdmin, async (req, res) => {
  try {
    const { type, id } = req.params;
    ...
    switch (type) {
      case 'bus':
        ...
        break;
      case 'driver':
        query = 'SELECT profile_photo_url FROM users WHERE id = $1 AND role = $2';
        params = [id, 'driver'];
        break;
      case 'route':
    ...
    const fileUrl = result.rows[0][`${type}_image_url`] || result.rows[0][`${type}_map_url`] || result.rows[0].profile_photo_url;
...
router.get('/list/:folder', authenticateUser, requireAdmin, async (req, res) => {
  try {

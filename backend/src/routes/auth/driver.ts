...
const fetchShiftDetails = async (
...
): Promise<{ id: string; name: string | null; start_time: string | null; end_time: string | null } | null> => {
  ...
  const { data, error } = await supabaseClient
    ...
    .maybeSingle();
  if (error) {
    logger.warn('Error fetching shift details', 'auth', { error: error.message, shiftId });
    return null;
  }
  if (!data) {
    logger.warn('Shift not found', 'auth', { shiftId });
    return null;
  }
  return {
    id: data.id,
...
router.post('/driver/login', async (req, res) => {
  try {
    ...
    if (profile.role !== 'driver') {
      logger.warn('❌ Non-driver attempted driver login', 'auth', { userId: authData.user.id, email, role: profile.role });
      return res.status(403).json({
        success: false,
        error: 'Access denied',
    ...
    if (busError || !busData) {
      if (busError?.code === 'PGRST116') {
        logger.warn('❌ No bus assignment found for driver', 'auth', { userId: authData.user.id, email });
      ...
      logger.error('❌ Error fetching bus assignment', 'auth', { userId: authData.user.id, error: busError?.message });
      return res.status(500).json({
        success: false,
    ...
    logger.error('❌ Driver login error', 'auth', { error: error instanceof Error ? error.message : 'Unknown error' });
    return res.status(500).json({
      success: false,
...
router.get('/driver/assignment', async (req, res) => {
  try {
    ...
    logger.info('✅ Bus assignment retrieved successfully', 'auth', { bus_number: bus.bus_number, route_name: routeName });
...
router.post('/driver/validate', async (req, res) => {
  try {
    ...
    if (error || !user) {
      logger.warn('❌ Token validation failed', 'auth', { error: error?.message });
      return res.status(401).json({
        success: false,
        error: 'Invalid token',
    ...
    const profile = profile2 as { id: string; full_name: string | null; role: 'driver' | 'student' | 'admin'; is_active: boolean; } | null;
    ...
    logger.error('❌ Session validation error', 'auth', { error: error instanceof Error ? error.message : 'Unknown error' });
    return res.status(500).json({
      success: false,
      error: 'Internal server error',

...
router.post('/driver/login', async (req, res) => {
    try {
        ...
        if (authError) {
            logger_1.logger.warn('❌ Driver authentication failed', 'auth', {
                email,
        ...
        logger_1.logger.info('✅ Supabase authentication successful', 'auth', {
        ...
        if (profileError || !profile) {
            logger_1.logger.error('❌ Failed to fetch user profile', 'auth', {
                userId: authData.user.id,
        ...
        if (!profile.is_active) {
            logger_1.logger.warn('❌ Inactive account attempted login', 'auth', {
                userId: authData.user.id,
                email
            });
        ...
        if (profile.role !== 'driver') {
            logger_1.logger.warn('❌ Non-driver attempted driver login', 'auth', {
                userId: authData.user.id,
                email,
                role: profile.role
            });
      ...
      `)
        ...
        try {
            ...
            logger_1.logger.warn('⚠️ Failed to update last login time', 'auth', {
                userId: authData.user.id,
        ...
        logger_1.logger.error('❌ Driver login error', 'auth', {
...
router.get('/driver/assignment', async (req, res) => {
    try {
        ...
        logger_1.logger.info('✅ Bus assignment retrieved successfully', 'auth', {
            bus_number: busData.bus_number,
            route_name: routeName
        });
        return res.json({
...
router.post('/driver/validate', async (req, res) => {
    try {
        ...
        if (error || !user) {
            logger_1.logger.warn('❌ Token validation failed', 'auth', {
                error: error?.message
            });
            return res.status(401).json({
                success: false,
      ...
      `)
        ...
        logger_1.logger.error('❌ Session validation error', 'auth', {
            error: error instanceof Error ? error.message : 'Unknown error'
        });
        return res.status(500).json({
            success: false,
...
router.post('/student/login', async (req, res) => {
    try {
        ...
        if (authError) {
            logger_1.logger.warn('❌ Student authentication failed', 'auth', {
                email,
                error: authError.message
            });
            let errorMessage = 'Invalid credentials';
        ...
        if (profile.role !== 'student') {
            logger_1.logger.warn('❌ Non-student attempted student login', 'auth', {
                userId: authData.user.id,
                email,
                role: profile.role
            });
        ...
        logger_1.logger.error('❌ Student login error', 'auth', {
            error: error instanceof Error ? error.message : 'Unknown error'
        });
        return res.status(500).json({
            success: false,
